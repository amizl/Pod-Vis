      ...
      <g>
        {
          ...
          .map((label) =>
            <g transform={ `translate(${this.scale(label.toString())}, 0)` }>
              <rect x='0' y='0' width='10' height='10' fill={ this.colorScale(label) }/>
              <text x='20' y='10' fill='black'>{ label }</text>
            </g>)
        }
      </g>
    )
...
export default {
  ...
  computed: {
    ...
    colorScale() {
      return scaleOrdinal(schemeGreys[6])
        .domain([0, 1, 2, 3, 4, 5]); // TODO: need better way than to hardcode domain
    },
    stack() {
      return stack()
  ...
  render(h) { // eslint-disable-line no-unused-vars
    const bars = this.stack(this.data)
      .map(cohort =>
