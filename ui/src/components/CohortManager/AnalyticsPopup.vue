        ...
        <v-container fluid fill-width class="ma-2 pa-0 mx-2">
          ...
          >
            <v-col cols="4" class="ma-0 pa-0">
              <v-chip label :color="c.color" class="ma-1"
                ><span class="white--text font-weight-bold">{{
                  c.subject_ids ? c.subject_ids.length : '?'
                }}</span></v-chip
              >
              {{ c.label }}
            </v-col>
          ...
          <v-row v-if="collection.is_longitudinal" class="ma-0 pa-0">
            <v-col cols="12" class="ma-0 pa-0">
              <span class="primary--text title"
                >Analysis Measure:
                <span class="black--text">{{ comparisonMeasure }}</span></span
              >
            </v-col>
          </v-row>
          ...
          <v-row class="ma-0 pa-0 mb-3">
            <v-col cols="12" class="ma-0 pa-0" align="left">
              <div
                v-if="
                  pvals_request_status == null ||
                    pvals_request_status == 'loading'
                "
              >
  ...
  methods: {
    ...
    async saveCohorts(cohorts, cohortPrefix) {
      ...
      for (var c = nCohorts - 1; c >= 0; c--) {
        ...
        var queries = {};
        queries[cohort.dimension] = [
          {
            minValue: cohort.range.min * 1.0,
            maxValue: cohort.range.max * 1.0,
          },
        ];
        var args = {
