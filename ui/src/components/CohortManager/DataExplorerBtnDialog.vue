  ...
  <div>
       <v-tooltip bottom color="primary">
         <template v-slot:activator="{ on }">
          <v-btn color="primary--text" @click="dialog = !dialog" :disabled="collection_cohorts.length == 0" v-on="on">
      ...
      <v-card class="rounded-lg">
        <v-card-title class="title primary--text text--darken-3">
          <span class="primary--text title pl-2"
            >Are you sure you want to leave the Cohort Manager and go to the Data Explorer?</span
          >
        </v-card-title>
        <v-divider></v-divider>
        <v-card-actions>
          ...
          <v-btn :loading="loading" color="primary" @click="routeToDataExplorer">
            Yes</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
