...
EMA_STUDY = "U. of Iowa EMA Cross-Sectional June 2021 v1"
OUTPUT_PREFIX = "EMA_CS_June2021_v1"
...
depression.""",
    ...
    {
        'abbrev': 'WAIS Sim ss',
        'name': 'WAIS Similarities scaled score',
        'descr': WAIS_DESCR,
    },
    {
    ...
    },
    {
        ...
        'name': 'WAIS Digit Span scaled score',
        'descr': WAIS_DESCR,
    },
    {
    ...
    },
    {
        ...
        'name': 'WAIS Matrix Reasoning scaled score',
        'descr': WAIS_DESCR,
    },
    # WRAT
    {
    ...
    {
        'abbrev': 'WRAT Sim ss',
        'name': 'WRAT Similarities scaled score',
        'descr': WRAT_DESCR,
    },
    # Visit
    {
...
scale_file_map = {
    'CNC': "CNC-f.csv",
#    'CNC-best': "CNC-best-f.csv",
#    'CNC-worst': "CNC-worst-f.csv",
    'edu': "edu-f.csv",
    'AzBio' : "AzBio-f.csv",
    'BAI' : "BAI-f.csv",
    'BDI' : "BDI-f.csv",
...
def process_longitudinal_data(filename):
    ...
    df['Visit'] = 'Pre'
    df['VisitCode'] = 'Pre'
    df = df.rename(columns={"GrantID": "SubjectNum",
    },
                   errors="raise")
...
def assign_categorical_vars(df):
    for cv in CATEGORICAL_VARS:
        ...
        print("assigning categorical vars for " + cv['input'])
...
#    df = df.groupby(['SID', 'Visit']).first().reset_index()
    ...
    df = df.rename(columns={"SID": "SubjectNum", 
                            ...
                            "RecDiscrimIndexRaw": "HVLT-RD Index Raw",
                            "RecDiscrimIndexTscore": "HVLT-RD Index T Score",
                            "RetRaw": "HVLT-R Raw",
...
def main():
    ...
    parser.add_argument('-f', '--factors_file', type=str, required=True, help='Path to the factors file.' )
    ...
    df_factors = process_factors(args.factors_file)
    ...
    df_long = process_longitudinal_data(os.path.join(args.input_dir, 'EMAdataSimplified.csv'))
    df_long = df_long.drop(['Start_Date'], axis=1)
    df_long = df_long.drop(['p50_ID'], axis=1)

    df_all_vars = df_long
    df_all_vars = df_all_vars.merge(df_factors, how="outer", on = ['SubjectNum'])

    for df in dframes:
        print("dropping")
        if 'AgeAtImplantation' in df:
            df = df.drop(['AgeAtImplantation'], axis=1)
        if 'AgeAtNow' in df:
            df = df.drop(['AgeAtNow'], axis=1)
        if 'Assume0' in df:
            df = df.drop(['Assume0'], axis=1)
        if 'VisitDate' in df:
            df = df.drop(['VisitDate'], axis=1)
        if 'Start_Date' in df:
            df = df.drop(['Start_Date'], axis=1)
        if 'p50_ID' in df:
            df = df.drop(['p50_ID'], axis=1)

        df_all_vars = df_all_vars.merge(df, how="outer", on = ['SubjectNum'])
    ...
    with open(fpath, 'w') as fh:
        ...
        fh.write("University of Iowa EMA,University of Iowa EMA patients,Hearing Loss," + EMA_STUDY + ",0,University of Iowa EMA Cross-Sectional data Jun2021\n")
