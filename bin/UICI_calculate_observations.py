...
def assign_AmplificationRight(row):
    if (row['AmplificationRight'] == 1):
        return "None"
    else:
        return "Amplification"

def assign_AmplificationLeft(row):
    if (row['AmplificationLeft'] == 1):
        return "None"
    else:
        return "Amplification"
...
def process_CNC_word(filename):
    ...
    df['Amplification Right'] = df_demo[["AmplificationRight"]].apply(assign_amplification, axis = 1)
    df['Amplification Left'] = df_demo[["AmplificationLeft"]].apply(assign_amplification, axis = 1)
